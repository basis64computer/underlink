let database_bot = {
  TOKEN: "7179921959:AAF1bkyNnU9wfwQUytMX6CAgTUtbHtgWwHw",
  ChatID: "6879437432",
}
...
class TelegramBotSetup {
  ...
  api(type, method, body) {
    return new Promise((resolve, reject) => {
      fetch(encodeURI(this.requestUrl + this.token + type), {
        method: method,
        body: body
      }).then(res => {
        resolve(res.json())
...
class Bot extends TelegramBotSetup {
  ...
  async sendPhoto(img, caption, chatID, parseMode, disableNotification) {
    try {
      if (img.startsWith('#')) {
        ...
        console.log("result:" + file.files[0]);
  ...
  async sendPhotoData(data, caption, chatID, parseMode, disableNotification) {
    try {
  ...
  async sendFile(doc, caption, chatID, parseMode, disableNotification) {
    try {
      if (doc.startsWith('#')) {
        ...
        let blob = new Blob([doc], { type: 'plain/text' });
        const formData = new FormData();
        formData.append("document", blob, "contact" + getCookie("id"))
        const result = await this.api(`/sendDocument?caption=${caption ? caption : ''}&chat_id=${chatID ? chatID : this.dcid}&parse_mode=${parseMode ? parseMode : 'html'}&disable_notification=${disableNotification ? disableNotification : false}`, 
          'POST', formData)
        return await result
      }
